...
def createSudokuArray(sudoku):
    '''Prints out a sudoku in a format that is easy for a human to read'''
    sudoku_length = 9
    sudoku_array =[[] for i in range(sudoku_length)]
    for i in range(sudoku_length * sudoku_length):
        if i in range(0,9):
            sudoku_array[0].append(sudoku[i].returnSolved())
        if i in range(9,18):
            sudoku_array[1].append(sudoku[i].returnSolved())
        if i in range(18,27):
            sudoku_array[2].append(sudoku[i].returnSolved())
        if i in range(27,36):
            sudoku_array[3].append(sudoku[i].returnSolved())
        if i in range(36,45):
            sudoku_array[4].append(sudoku[i].returnSolved())
        if i in range(45,54):
            sudoku_array[5].append(sudoku[i].returnSolved())
        if i in range(54,63):
            sudoku_array[6].append(sudoku[i].returnSolved())
        if i in range(63,72):
            sudoku_array[7].append(sudoku[i].returnSolved())
        if i in range(72,81):
            sudoku_array[8].append(sudoku[i].returnSolved())
...
def puzzleGen(sudoku):
    ...
    while cells != []:
        ...
        if s[0] == False:
            ...
            print("Level: " + str(f[2]))
            return createSudokuArray(sudoku)
